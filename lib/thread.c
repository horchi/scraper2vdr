...
void *cThread::StartThread(cThread *Thread)
{
  ...
  {
     tell(0, "%s thread started (pid=%d, tid=%d, prio=%s)", Thread->description, getpid(), Thread->childThreadId, Thread->lowPriority ? "low" : "high");
#ifdef PR_SET_NAME
  ...
  if (Thread->description)
     tell(0, "%s thread ended (pid=%d, tid=%d)", Thread->description, getpid(), Thread->childThreadId);
