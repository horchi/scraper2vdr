...
int cSearchTimer::getDoneFor(cDbRow* searchTimer, cDbRow* useevent, json_t* obj)
{
   ...
   useeventsDb->clear();
   useeventsDb->setValue("CNTEVENTID", useevent->getIntValue("CNTEVENTID"));
   ...
   if (!useeventsDb->find())
   {
      tell(0, "Warning: Event '%s/%ld/%s' not found", 
           useevent->getStrValue("CHANNELID"), 
           useevent->getIntValue("CNTEVENTID"), 
   ...
   if (ptyRecName && namingmode != tnmDefault)
   {
      ...
      if (ptyRecName->execute(useeventsDb, namingmode) == success)
      {
         ...
         if (!isEmpty(ptyRecName->getResult()))
            timerRow.setValue("FILE", connection->escapeSqlString(ptyRecName->getResult()).c_str());
      }
   }
